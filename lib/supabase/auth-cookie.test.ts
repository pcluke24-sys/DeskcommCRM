...
describe("PKCE sem relaxar a sessão", () => {
  it("o verificador acompanha o link do e-mail", () => {
    expect(authCookieOptions("sb-auth-code-verifier", {sameSite: "strict", httpOnly: true, secure: true}))
      .toEqual({sameSite: "lax", httpOnly: true, secure: true});
  });
  it.each(["sb-auth", "sb-auth.0", "active_org"])("mantém Strict para %s", name => {
