...
const estado = vi.hoisted(() => ({ podeMover: true, iaLigada: false }));
...
vi.mock("@/hooks/auth/AuthProvider", () => ({
  useAuth: () => ({ activeOrg: { ai_module_enabled: estado.iaLigada } }),
...
vi.mock("@/components/kanban/LeadQualificationDialog", () => ({ LeadQualificationDialog: () => null }));
...
describe("menu do card — Excluir", () => {
  it("mantém exclusão disponível sem mostrar qualificação quando a IA está bloqueada", async () => {
    const user = userEvent.setup();
    renderMenu();
    await user.click(screen.getByRole("button", { name: "Ações do lead" }));
    expect(await screen.findByRole("menuitem", { name: "Excluir" })).toBeVisible();
    expect(screen.queryByRole("menuitem", { name: "Analisar qualificação com IA" })).not.toBeInTheDocument();
  });
